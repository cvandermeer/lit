class QuestionsController < ApplicationController
  ...
  def submit_answer
    ...
    if answer.id == @question.correct_answer_id
      ...
      current_user.save
      UserResult.create(user: current_user, category: @question.category,
                        language: @question.language, correctly_answered: true)
    else
      UserResult.create(user: current_user, category: @question.category,
                        language: @question.language, correctly_answered: false)
    end
  ...
  def question_params
    params.require(:question).permit(:title, :image,
                                     :image_cache, :language_id,
                                     :category_id, answers_attributes: [:id, :title,
                                                                        :image, :image_cache,
                                                                        :_destroy])
  end
